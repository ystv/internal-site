...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (typeof csrfCookie !== "string" || csrfCookie === null) {
    return new NextResponse("Something went wrong - please try again. (No CSRF token in cookie.)", {
      status: 400,
    });
  }
  ...
  if (dataRaw.get("g_csrf_token") !== csrfCookie) {
    return new NextResponse("Something went wrong - please try again. (CSRF token mismatch.)", {
      status: 400,
    });
  }
  const idToken = dataRaw.get("credential");
  if (typeof idToken !== "string" || idToken === null) {
    return new NextResponse("Something went wrong - please try again. (No credential in Google response.)", {
      status: 400,
    });
  }
