...
async function getKey() {
  if (!key) {
    key = await crypto.subtle.importKey("raw", new TextEncoder().encode(process.env.SESSION_SECRET), {
      name: "HMAC",
      hash: "SHA-256",
    }, false, ["sign", "verify"]);
  }
...
export async function createICalTokenForUser(userID: number) {
  ...
  const signature = await crypto.subtle.sign({
    name: "HMAC",
    hash: "SHA-256"
  }, await getKey(), new TextEncoder().encode(payload));
  return hexEncode(signature) + "." + hexEncode(new TextEncoder().encode(payload));
}
...
export async function getUserFromICalToken(tok: string): Promise<number> {
  ...
  if (!(await crypto.subtle.verify({
    name: "HMAC",
    hash: "SHA-256"
  }, await getKey(), hexDecode(signature), hexDecode(encodedPayload)))) {
    throw new Error("Invalid signature");
  }
  const { userID } = JSON.parse(new TextDecoder().decode(hexDecode(encodedPayload)));
  return userID;
}
